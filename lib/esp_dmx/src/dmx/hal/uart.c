...
#endif
    default: return PERIPH_UART0_MODULE;  // unreachable, dmx_num already validated by caller
  }
}
...
bool dmx_uart_init(dmx_port_t dmx_num, void *isr_context, int isr_flags) {
  ...
  periph_module_enable(dmx_uart_get_periph_module(dmx_num));
  if (dmx_num != 0) {  // Default UART port for console
    ...
    periph_module_reset(dmx_uart_get_periph_module(dmx_num));
